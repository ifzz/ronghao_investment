...
void strategy_demo2::execute(depth_dia_group& group) {
	...
	case STATUS_READY: {		//此处产生第一次开仓行为
		if ((ext->close_item.price > ext->open_item.price+data.m_last_amplitude) ||
				(ext->close_item.price < ext->open_item.price-data.m_last_amplitude)) {
			if (ext->close_item.price > ext->open_item.price+data.m_last_amplitude) {		//做多
				execute_trade(group, FLAG_OPEN, DIRECTION_BUY);
				data.m_sts = STATUS_OPEN_BUY;
			} else if (ext->close_item.price < ext->open_item.price-data.m_last_amplitude) {		//做空
