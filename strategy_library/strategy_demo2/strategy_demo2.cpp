...
void strategy_demo2::execute_trade(depth_dia_group& group,
...
void strategy_demo2::execute(depth_dia_group& group) {
	...
	MarketAnalyseKline *ext = group.diagrams[m_1minkline_index].ext;
//	print_thread_safe("[strategy_demo2]当前这个包的日期为：%d，序号：%d，最高价：%d，最低价：%d，"
...
//			ext->min_item.price, ext->open_item.price, ext->close_item.price);
	...
	if (m_sts != STATUS_INIT) {
		if (base->_seq > m_last_k_line._seq) {
			m_last_amplitude = m_last_k_line.m_max_price-m_last_k_line.m_min_price;
	...
	case STATUS_READY: {		//此处产生第一次开仓行为
		if ((ext->close_item.price > ext->open_item.price+m_last_amplitude) ||
				(ext->close_item.price < ext->open_item.price-m_last_amplitude)) {
			if (ext->close_item.price > ext->open_item.price+m_last_amplitude) {		//做多
				execute_trade(group, FLAG_OPEN, DIRECTION_BUY);
				m_sts = STATUS_OPEN_BUY;
			} else if (ext->close_item.price < ext->open_item.price-m_last_amplitude) {		//做空
