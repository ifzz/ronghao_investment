 ...
 */
 ...
 void sideways_false_breakthrough::check_whether_satisfy_status(int64_t tick, ins_data& data) {
	 ...
	 for (auto& group : data.m_group_queue) {
		 MarketAnalyseKline *in = group.diagrams[m_1minkline_index].ext;
		 if (in->max_item.price > data.m_middle_price+5*tick || in->min_item.price < data.m_middle_price-5*tick) {
	 ...
	 MarketAnalyseDataBase *first_base = data.m_group_queue.front().diagrams[m_1minkline_index].base;
	 MarketAnalyseKline *first_ext = data.m_group_queue.front().diagrams[m_1minkline_index].ext;
	 MarketAnalyseDataBase *last_base = data.m_group_queue.back().diagrams[m_1minkline_index].base;
	 MarketAnalyseKline *last_ext = data.m_group_queue.back().diagrams[m_1minkline_index].ext;
 ...
 void sideways_false_breakthrough::construct_20_kline(depth_dia_group& group, int64_t tick, ins_data& data) {
	 data.m_group_queue.push_back(group);

	 MarketAnalyseKline *ext = group.diagrams[m_1minkline_index].ext;
	 if (ext->max_item.price > data.m_max_highest_price)
		 data.m_max_highest_price = ext->max_item.price;		//记录最高价
	 else if (ext->min_item.price < data.m_min_lowest_price)
		 data.m_min_lowest_price = ext->min_item.price;			//记录最低价

	 if (data.m_group_queue.size() == 20)		//已取满20个K线节点，检测这些节点是否满足状态
		 check_whether_satisfy_status(tick, data);
 }

 void sideways_false_breakthrough::reselect_20_kline(depth_dia_group& group, int64_t tick, ins_data& data) {
	 data.m_20_kline_is_ready = false;
	 data.m_node_sts = STATUS_INIT;
	 data.m_group_queue.pop_front();

	 if (2 == group.diagrams[m_1minkline_index].base->_state)		//当前节点是一个已完成节点
		 construct_20_kline(group, tick, data);
 }
 ...
 void sideways_false_breakthrough::execute(depth_dia_group& group) {
	 ...
	 if (!data.m_offset_open) {
		 ...
		 if (STATUS_INIT == data.m_node_sts) {
			 ...
			 if (ext->close_item.price > data.m_middle_price+5*tick) {
				 data.m_node_sts = STATUS_BT_M_PLUS_FIVE;			//收盘价突破m+5
				 data.m_open_highest = ext->close_item.price;
				 print_thread_safe("「sideways_false_breakthrough」当前这根K线从初始化状态突破m+5=%d...\n",
						 data.m_middle_price+5*tick);
			 } else if (ext->close_item.price < data.m_middle_price-5*tick) {
				 data.m_node_sts = STATUS_BT_M_REDUCE_FIVE;		//收盘价突破m-5
				 data.m_open_lowest = ext->close_item.price;
				 print_thread_safe("「sideways_false_breakthrough」当前这根K线从初始化状态突破m-5=%d...\n",
						 data.m_middle_price-5*tick);
			 }

		 } else if (STATUS_BT_M_PLUS_FIVE == data.m_node_sts) {		//之前已突破m+5
			 if (ext->max_item.price > data.m_open_highest) {
				 print_thread_safe("「sideways_false_breakthrough」当前这根K线最高价的最大值从%d突破到%d...\n", data.m_open_highest, ext->max_item.price);
				 data.m_open_highest = ext->close_item.price;
				 //在m+5和m_open_highest(h)之间波动时不做任何操作
			 }

			 if ( (ext->close_item.price < data.m_middle_price+5*tick) && (ext->close_item.price > data.m_middle_price-25*tick) ) {
				 //这一收盘价又回落到(m-25, m+5)之间，那么再检测(持仓量)成交量是否满足条件

				 if (base->_volume_tick < 3*data.m_avg_volume) {		//成交量小于均值的3倍，发出开仓行为（做空）
					 ...
					 if (ext->close_item.price > data.m_middle_price)
					 ...
					 reselect_20_kline(group, tick, data);
					 return;
				 }

			 } else if (ext->close_item.price <= data.m_middle_price-25*tick) {
				 //直接重新选取20个K线节点
			 ...
			 if (ext->min_item.price < data.m_open_lowest) {
				 print_thread_safe("「sideways_false_breakthrough」当前这根K线最低价的最小值从%d降到%d...\n", data.m_open_lowest, ext->min_item.price);
				 data.m_open_lowest = ext->close_item.price;
			 ...
			 if (ext->close_item.price > data.m_middle_price-5*tick && ext->close_item.price < data.m_middle_price+25*tick) {
				 ...
				 if (base->_volume_tick < 3*data.m_avg_volume) {		//成交量小于均值3倍，发出开仓行为（做多）
					 data.m_offset_open = true;
		 ...
		 if (2 == base->_state && !data.m_offset_open) {			//在这个节点完成过程中中没有发生开仓行为
		 ...
		 if (STATUS_BT_M_PLUS_FIVE == data.m_node_sts) {
			 ...
			 if (ext->close_item.price < data.m_middle_price-25*tick) {
				 //直接平仓，止盈
				 data.m_offset_open = false;
			 } else if (ext->close_item.price > data.m_open_highest) {
				 //直接平仓，止损
				 data.m_offset_open = false;
			 } else if (ext->close_item.price > data.m_middle_price) {
			 ...
			 if (ext->close_item.price > data.m_middle_price+25*tick) {
				 //高于m+25，直接平仓，止盈
				 data.m_offset_open = false;
			 } else if (ext->close_item.price < data.m_open_lowest) {
				 //低于l，直接平仓，止损
				 data.m_offset_open = false;
			 } else if (ext->close_item.price < data.m_middle_price) {
