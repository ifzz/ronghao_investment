...
void strategy_base::print_thread_safe(const char *format, ...) {
	...
	va_end(args);
	auto job = std::make_shared<log_job>(impl->format_buf, &impl->m_log, g_conf.print_screen);
	g_log_th.job_dispatch(job);
}
...
void sb_impl::on_init() {
	...
	strategy_config += "\n###############################################################\n";
	print_thread_safe(g_log, strategy_config.c_str());

	file_dia.data.data_base = new MarketAnalyseDataBase;
	file_dia.data.ext.data = (char*)new MarketAnalyseKline;
	file_dia.data.ext.len = sizeof(MarketAnalyseKline);
	for (int i = 0; i < 32; ++i) {
		dia_item item;
		item.tag_base = new MarketAnalyseTagBase;
		item.ext.data = (char*)new MarketAnalyseDpo;
		item.ext.len = sizeof(MarketAnalyseDpo);
		file_dia.tags.push_back(item);
	}

	m_this_stg->init();

	delete file_dia.data.data_base;
	delete (MarketAnalyseKline*)file_dia.data.ext.data;
	for (auto& tag : file_dia.tags) {
		delete tag.tag_base;
		delete (MarketAnalyseDpo*)tag.ext.data;
	}
}

void sb_impl::record_trade(const std::string& buf, unsigned int trade_date) {
	if (m_current_date != trade_date) {		//每天新建一个文件，将当日所有下单记录保存在同一个文件中
		if (m_trade_import)		//首先将上一日的文件关闭
			fclose(m_trade_import);

		m_current_date = trade_date;
		std::string file = "database_import/";			//测试环境直接用默认的导入目录
		if (g_conf.for_produce)
			file = g_conf.db_import+"/";
		file += std::to_string(m_current_date)+"/";

		if (access(file.c_str(), F_OK))		//若该目录还不存在，则先创建该目录
			mkdir(file.c_str(), 0755);

		file += std::to_string(m_stg_id)+".log";
		m_trade_import = fopen(file.c_str(), "w");		//再新开一个文件存储
		m_this_stg->print_thread_safe("日期更新，新建一个当前策略的买卖点日志current date = %d, file name = %s, "
				"file ptr=%p\n", m_current_date, file.c_str(), m_trade_import);
	}
	fprintf(m_trade_import, "%s", buf.c_str());
	fflush(m_trade_import);
}

void strategy_base::request_trade(const std::string& ins_id, int data_index, order_instruction& oi) {
	sb_impl *impl = static_cast<sb_impl*>(m_obj);
	...
	auto& dia_name = g_data_tag_info.m_type_dia[data_index];
	memcpy(oi.dia_name, dia_name.c_str(), dia_name.size());
	oi.dia_name[dia_name.size()] = '\0';
	...
	sprintf(&buf[0], "本地%d#%09d %d %d %d  %s  %s#%d  %s#%s  %lld#%d#%d  行情%d#%09d\n",
			oi.uuid.date, oi.uuid.time, oi.uuid.strategy_id, oi.uuid.src, oi.uuid.seq, ins_id.c_str(), oi.dia_name, oi.dia_seq,
			offset_flag.c_str(), trade_direction.c_str(), oi.price, oi.vol_cnt, oi.level, oi.market.date, oi.market.time);
	print_thread_safe("%s", buf.c_str());
	impl->record_trade(buf, oi.uuid.date);
}
...
void strategy_base::for_each_his_dia(const std::string& id, long param, const std::string& name, const std::string& class_name,
	...
	//历史文件
	impl->file_dia.data.data_base->_date = date;
	impl->file_dia.data.data_base->_seq = 0;
	...
	cache.cache_dia[data_index].data.mode = -1;
	for (int i = 0; i < 32; ++i) {
		dia_item item;
		item.mode = -1;
		cache.cache_dia[data_index].tags.push_back(item);
	}
...
void ss_util::parse_ini(bool is_service) {
	E15_Ini ini;
	...
	if (is_service)
		g_conf.print_screen = false;
	else
		g_conf.print_screen = true;
	...
	g_conf.so_local_dir = ini.ReadString("so_local_dir", "");
}

void ss_util::global_log_init(bool need_history_store) {
	//全局日志初始化
...
int ss_util::get_dia_history(void * obj,int market,const char * id,MarketDataType * dt,MarketDataType * tt,
	...
	if (!tt) {		//当前得到的是一个data
		if (1 == impl->file_dia.data.mode) {
			auto& cache = impl->cache[id];
			cache.f(impl->file_dia, cache.args);
		}

		*impl->file_dia.data.data_base = *base->data_base;
		if (!impl->file_dia.data.data_base->_state)
			impl->file_dia.data.data_base->_state = 2;
		if (len && len == sizeof(MarketAnalyseKline))
			memcpy(impl->file_dia.data.ext.data, ext_data, len);
		impl->file_dia.data.mode = 1;
		for (auto& tag : impl->file_dia.tags)
			tag.mode = -1;
	} else {		//得到的是一个tag
		...
		*impl->file_dia.tags[tag_idx].tag_base = *base->tag_base;
		if (len)
			memcpy(impl->file_dia.tags[tag_idx].ext.data, ext_data, len);
		impl->file_dia.tags[tag_idx].mode = 1;
	}
	return 1;
}
...
void ss_util::load_dia_hiscache(const std::string& id, int data_index, DiagramDataHandler *h, short store_level, void *obj) {
	...
	if (-1 == impl->file_dia.data.mode) {		//没有加载到历史文件，一直加载直到缓存的最后第二根K线
		wd_seq fseq, cseq;
		fseq.date = impl->file_dia.data.data_base->_date;
		fseq.seq = impl->file_dia.data.data_base->_seq;
	...
	if (1 == impl->file_dia.data.mode) {
		while (true) {
			if (item->base._date == impl->file_dia.data.data_base->_date &&
					item->base._seq == impl->file_dia.data.data_base->_seq)		//在缓存中找到历史文件中的最后一根K线
				break;
...
void ss_util::load_cache_forstg(const std::string& id, int data_index, DiagramDataItem *item, sb_impl *impl, bool exec) {
	auto& cache = impl->cache[id];
	cache.cache_dia[data_index].data.data_base = &item->base;
	if (item->pri && item->pri->Length() == sizeof(MarketAnalyseKline)) {
		cache.cache_dia[data_index].data.ext.data = (char*)item->pri->c_str();
		cache.cache_dia[data_index].data.ext.len = sizeof(MarketAnalyseKline);
	}
	cache.cache_dia[data_index].data.mode = 1;
	for (auto& tag : cache.cache_dia[data_index].tags)
		tag.mode = -1;
	...
	for (int i = 0; i < item->tag_cnt; ++i) {
		...
		cache.cache_dia[data_index].tags[i].tag_base = &tag->base;
		cache.cache_dia[data_index].tags[i].mode = 1;
	}
	if (exec)
	...
	for (auto& info : m_ins_info) {
		...
		for (unsigned long i = 0; i < stock->factory->m_data->Count(); ++i) {
			...
			for (size_t j = 0; j < h->m_data->Count(); ++j) {
				...
				//base & ext
				group.data.data_base = &item->base;
				if (!group.data.data_base->_state)
					group.data.data_base->_state = 2;
				group.data.ext.data = (char*)item->pri->c_str();
				group.data.ext.len = sizeof(MarketAnalyseKline);
				group.data.mode = 1;
				...
				for (int k = 0; k < item->tag_cnt; ++k) {
					...
					if (tag) {
						it.tag_base = &tag->base;
						it.ext.data = (char*)tag->pri->c_str();
						it.ext.len = tag->pri->Length();
						it.mode = 1;
					}
					group.tags.push_back(it);
				}
	...
	*group.depth = *stock->depth;
	...
	if (dia_vt) {
		...
		for (auto& raw : g_dia_deq) {
			dia_group g;
			g.data.data_base = &raw.data->base;
			g.data.ext.data = (char*)raw.data->pri->c_str();
			g.data.ext.len = sizeof(MarketAnalyseKline);
			g.data.mode = raw.mode;

			for (int i = 0; i < raw.data->tag_cnt; ++i) {
				dia_item it;
				if (raw.data->tags[i]) {		//tag存在，删除操作也要通知策略
					it.tag_base = &raw.data->tags[i]->base;
					if (raw.data->tags[i]->pri->Length()) {
						it.ext.data = (char*)raw.data->tags[i]->pri->c_str();
						it.ext.len = raw.data->tags[i]->pri->Length();
					}
					if (raw.tag_mode.end() != raw.tag_mode.find(i))
						it.mode = raw.tag_mode[i];
				}
				g.tags.push_back(it);
			}
		...
		for (auto& dia : group.dias) {
			...
			dia.second.sort([](const dia_group& i, const dia_group& j)->bool {
				wd_seq iseq, jseq;
				iseq.date = i.data.data_base->_date;
				iseq.seq = i.data.data_base->_seq;
				jseq.date = j.data.data_base->_date;
				jseq.seq = j.data.data_base->_seq;
